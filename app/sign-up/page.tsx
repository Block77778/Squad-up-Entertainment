      ...
      <Layout>
        <div className="min-h-screen flex items-center justify-center px-4 py-20">
          <div className="text-center max-w-lg mx-auto animate-scale-in">
            ...
            <h1 className="text-4xl md:text-5xl font-serif font-black mb-4 bg-clip-text text-transparent"
              style={{ backgroundImage: GRADIENT }}>
              Welcome to the Squad!
            </h1>
            <p className="text-text-secondary mb-8 leading-relaxed">
              Your account has been created, <strong className="text-white">{form.username}</strong>.
              You're now part of the Squad Up community. Check your email to verify your account and start competing.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link
                href="/membership"
                className="inline-block font-mono uppercase tracking-widest text-white text-sm px-8 py-3 rounded-lg font-bold transition-all hover:opacity-90 border border-white/20"
      ...
      <div className="min-h-screen flex items-center justify-center px-4 py-24">
        <div className="w-full max-w-lg">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl md:text-4xl font-serif font-black bg-clip-text text-transparent"
              style={{ backgroundImage: GRADIENT }}>
              Create Your Account
            </h1>
            ...
            {[1, 2].map((s) => (
              <React.Fragment key={s}>
                <div
                  ...
                  style={{
                    background: step >= s ? GRADIENT : 'rgba(255,255,255,0.05)',
                    color: 'white',
                    boxShadow: step >= s ? '0 2px 12px rgba(139,92,246,0.4)' : 'none',
                  }}
                >
                  {s}
                </div>
                {s < 2 && (
                  <div className="w-16 h-0.5 rounded-full transition-all"
                    style={{ background: step > s ? GRADIENT : 'rgba(255,255,255,0.1)' }} />
                )}
              </React.Fragment>
            ))}
          </div>
          ...
          <Card border="subtle" className="p-8">
              ...
              {step === 1 && (
                <div className="space-y-4 animate-fade-in">
                  ...
                  <div>
                    ...
                    <input className={inputClass('username')} placeholder="xX_SquadUp_Xx" value={form.username}
                  ...
                  <div>
                    ...
                    <input className={inputClass('email')} type="email" placeholder="john@example.com" value={form.email}
                  ...
                  <div>
                    ...
                    <input className={inputClass('password')} type="password" placeholder="Min 8 characters" value={form.password}
                  ...
                  <div>
                    ...
                    <input className={inputClass('confirmPassword')} type="password" placeholder="Repeat password" value={form.confirmPassword}
                      onChange={(e) => update('confirmPassword', e.target.value)} />
                    {errors.confirmPassword && <p className="text-red-400 text-xs mt-1">{errors.confirmPassword}</p>}
                  </div>

                  <button type="button" onClick={handleNext}
                    className="w-full font-mono uppercase tracking-widest text-white text-sm py-3 rounded-lg font-bold transition-all hover:opacity-90 hover:-translate-y-0.5 mt-2"
                    style={{ background: GRADIENT, boxShadow: '0 4px 20px rgba(139,92,246,0.4)' }}>
                    Next →
                  </button>
                </div>
              ...
              {step === 2 && (
                <div className="space-y-5 animate-fade-in">
                  ...
                  <div>
                    ...
                    <input className={inputClass('dob')} type="date" value={form.dob}
                      ...
                      {platforms.map((p) => (
                        <button key={p} type="button"
                          ...
                          style={form.platform === p
                            ? { background: GRADIENT, borderColor: 'transparent', color: 'white' }
                    ...
                    <div className="flex flex-wrap gap-2">
                      {games.map((g) => (
                        <button key={g} type="button"
                          ...
                          style={form.games.includes(g)
                            ? { background: GRADIENT_BG, borderColor: 'rgba(139,92,246,0.5)', color: 'white' }
                  ...
                  <div className="space-y-3 pt-2 border-t border-white/10">
                    <label className="flex items-start gap-3 cursor-pointer group">
                      <input type="checkbox" checked={form.agreeAge} onChange={(e) => update('agreeAge', e.target.checked)}
                  ...
                  <div className="flex gap-3 pt-1">
                    <button type="button" onClick={() => setStep(1)}
                      className="flex-1 font-mono uppercase tracking-widest text-white/60 text-sm py-3 rounded-lg font-bold border border-white/10 hover:border-white/20 transition-all">
                      ← Back
                    </button>
                    <button type="submit"
                      className="flex-[2] font-mono uppercase tracking-widest text-white text-sm py-3 rounded-lg font-bold transition-all hover:opacity-90 hover:-translate-y-0.5"
                      style={{ background: GRADIENT, boxShadow: '0 4px 20px rgba(139,92,246,0.4)' }}>
                      Create Account
                    </button>
                  </div>
            ...
            <p className="text-center text-xs text-text-muted mt-6">
              ...
              <Link href="/contact" className="text-violet-400 hover:text-violet-300 transition-colors">Sign in</Link>
            </p>
          </Card>
          ...
          <div className="mt-6 text-center">
            ...
            <Link href="/membership"
              className="inline-block text-xs font-mono uppercase tracking-widest px-4 py-2 rounded-full border border-white/15 text-white/60 hover:text-white hover:border-violet-500/50 transition-all"
              style={{ background: GRADIENT_BG }}>
              View Membership Tiers →
            </Link>
          </div>
